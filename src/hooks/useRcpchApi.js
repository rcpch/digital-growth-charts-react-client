        ...
        (newInput) => {
            setApiState((old) => {
                ...
                if (newInput.height_maternal && newInput.height_paternal) {
                    ...
                    mutable[mode].input[reference][measurementMethod].push(
                        newInput
                    );
                }
    ...
    useEffect(() => {
        ...
        if (apiState.isLoading) {
            ...
            if (apiState['isMidparentalCalculation']) {
                latestInput =
                    apiState[mode].input[reference]['parentalHeights'];
            } else {
                relevantArray =
                    apiState[mode].input[reference][measurementMethod];
            ...
            fetchFromApi(
                latestInput,
                reference,
                apiState['isMidparentalCalculation']
                    ? 'mid-parental-height'
                    : mode
            )
                .then((result) => {
                    if (!ignore) {
                        setApiState((old) => {
                            ...
                            if (mutable.isMidparentalCalculation) {
                                ...
                                mutable[mode].input[reference][
                                    'parentalheights'
                                ] = latestInput;
                                mutable[mode].output[reference][
                                    'midParentalHeights'
                                ] = result;
